	...
	double Cp, double Cn, int solver_type)
{
	...
	while (iter < max_iter)
	{
		...
		for (i=0; i<active_size; i++)
		{
			std::uniform_int_distribution<> randActiveSize(0, active_size - i - 1);
			int j = i + randActiveSize(mt);
			//int j = i+rand()%(active_size-i);
			swap(index[i], index[j]);
		}
...
double predict_probability(const struct model *model_, const struct feature_node *x, double* prob_estimates)
{
	...
	if(check_probability_model(model_))
	{
		...
		//probability based on decision value
		double label = predict_values(model_, x, prob_estimates);

		prob_estimates[0] = 0.5 + prob_estimates[0]/2;
		prob_estimates[1]=1.-prob_estimates[0];

		prob_estimates[0] = max(min(prob_estimates[0], 1.0), 0.0);
		prob_estimates[1] = max(min(prob_estimates[1], 1.0), 0.0);

		return label;
	} else {
		throw std::runtime_error("predict_probability is not supported in SVR");
	}
}
